...
from processor import FireworksProcessor
...
class Tester(Asisstant):
    ...
    def preguntar_usuario(self, question):
        ...
        self.confirmation = response.text[-1] == '1'
    ...
    def test(self):
        ...
        for key, value in tests.items():
            ...
            evaluator_prompt = (
                ...
                f"Otorga una puntuación entre 0 y 5 siguiendo esta escala:\n"
                f"5: Diagnóstico completamente correcto, preciso y bien argumentado.\n"
                f"4: Diagnóstico mayormente correcto, con pequeños errores o omisiones.\n"
                f"3: Diagnóstico parcialmente correcto, incluye elementos clave pero también errores importantes.\n"
                f"2: Diagnóstico en su mayoría incorrecto, pero con una idea lejana o vaga relacionada.\n"
                f"1: Diagnóstico equivocaddo, sin relación clara con el caso, pero sin riesgos críticos.\n"
            ...
            accuracy += int(response.text.split()[-1])
            cnt_tests += 5
